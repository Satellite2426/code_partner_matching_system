package com.satellite.usercenter.job;
...
public class PreCacheJob {
    ...
    public void doCacheRecommendUser() {
        RLock lock = redissonClient.getLock("hanpao:precachejob:docache:lock");
        try {
            // 只有一个线程能获取到锁
            if (lock.tryLock(0, 30000, TimeUnit.MILLISECONDS)) {
                for (Long userId : mainUserList) {
                    ...
                    String redisKey = String.format("hanpao:user:recommend:%s", userId);
