...
import com.satellite.usercenter.utils.AlgorithmUtils;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword, String planetCode) {
        ...
        if (!userPassword.equals(checkPassword)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "用户密码和检验密码不一致");
        }
        // 账户不能重复
        ...
        if (count > 0) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "用户星球编号重复");
        }
        ...
        if (!saveResult) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "用户信息保存错误");
        }
    ...
    public List<User> matchUsers(long num, User loginUser) {
        ...
        for (int i = 0; i < userList.size(); i++) {
            ...
            // 计算分数
            long distance = AlgorithmUtils.minDistanceTags(tagList, userTagList);
            list.add(new Pair<>(user, distance));

        }
        // 按照编辑距离(编辑距离越短, 相似度越高)由小到大排序
